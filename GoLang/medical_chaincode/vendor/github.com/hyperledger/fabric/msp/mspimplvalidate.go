...
func (msp *bccspmsp) validateIdentity(id *identity) error {
	...
	case *gm.CSP:
		validationChain, err := msp.getGMCertificationChainForBCCSPIdentity(id)
		if err != nil {
			return errors.WithMessage(err, "could not obtain certification chain")
		}

		err = msp.validateGMIdentityAgainstChain(id, validationChain)
		if err != nil {
...
func (msp *bccspmsp) validateGMCAIdentity(id *identity) error {
	...
	validationChain, err := msp.getGMUniqueValidationChain(id.gmcert, msp.getGMValidityOptsForCert(id.gmcert))
	if err != nil {
		return errors.WithMessage(err, "could not obtain certification chain")
	}
	if len(validationChain) == 1 {
	...
	return msp.validateGMIdentityAgainstChain(id, validationChain)
}
...
func (msp *bccspmsp) validateGMIdentityAgainstChain(id *identity, validationChain []*sm2.Certificate) error {
	return msp.validateGMCertAgainstChain(id.gmcert, validationChain)
}
...
func (msp *bccspmsp) validateGMCertAgainstChain(cert *sm2.Certificate, validationChain []*sm2.Certificate) error {
	// here we know that the identity is valid; now we have to check whether it has been revoked

	// identify the SKI of the CA that signed this cert
	SKI, err := getGMSubjectKeyIdentifierFromCert(validationChain[1])
	if err != nil {
		return errors.WithMessage(err, "could not obtain Subject Key Identifier for signer cert")
	}
...
func (msp *bccspmsp) getGMValidityOptsForCert(cert *sm2.Certificate) sm2.VerifyOptions {
...
// getSubjectKeyIdentifierFromCert returns the Subject Key Identifier for the supplied certificate
// Subject Key Identifier is an identifier of the public key of this certificate
func getGMSubjectKeyIdentifierFromCert(cert *sm2.Certificate) ([]byte, error) {
	var SKI []byte
