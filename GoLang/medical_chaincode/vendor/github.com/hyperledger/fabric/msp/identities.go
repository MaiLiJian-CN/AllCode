...
func newGMIdentity(cert *sm2.Certificate, pk bccsp.Key, msp *bccspmsp) (Identity, error) {
	if mspIdentityLogger.IsEnabledFor(zapcore.DebugLevel) {
		mspIdentityLogger.Debugf("Creating identity instance for cert %s", gmcertToPEM(cert))
	}
	...
	return &identity{id: id, gmcert: cert, pk: pk, msp: msp}, nil
}
...
func (id *identity) ExpiresAt() time.Time {
	switch id.msp.bccsp.(type) {
	case *gm.CSP:
		return id.gmcert.NotAfter
	default:
...
func (id *identity) GetOrganizationalUnits() []*OUIdentifier {
	...
	case *gm.CSP:
		...
		res := []*OUIdentifier{}
		for _, unit := range id.gmcert.Subject.OrganizationalUnit {
...
func (id *identity) Verify(msg []byte, sig []byte) error {
	...
	if mspIdentityLogger.IsEnabledFor(zapcore.DebugLevel) {
		mspIdentityLogger.Debugf("hashopt = %s", hashOpt.Algorithm())
	...
	switch id.msp.bccsp.(type) {
	case *gm.CSP:
		valid, err := id.msp.bccsp.Verify(id.pk, sig, msg, &sm2.GMOpts{UID: nil})
		if err != nil {
			return errors.WithMessage(err, "could not determine the validity of the gm signature")
		} else if !valid {
			return errors.New("The gm signature is invalid")
		}
	default:
...
func (id *identity) Serialize() ([]byte, error) {
	...
	switch id.msp.bccsp.(type) {
	case *gm.CSP:
		pb = &pem.Block{Bytes: id.gmcert.Raw, Type: "CERTIFICATE"}
	default:
...
func newGMSigningIdentity(cert *sm2.Certificate, pk bccsp.Key, signer crypto.Signer, msp *bccspmsp) (SigningIdentity, error) {
	//mspIdentityLogger.Infof("Creating signing identity instance for ID %s", id)
...
func (id *signingidentity) Sign(msg []byte) ([]byte, error) {
	...
	mspIdentityLogger.Debugf("Sign hashopt = %s", hashOpt.Algorithm())
	mspIdentityLogger.Debugf("Sign: digest: %X \n", digest)

	// Sign
	switch id.msp.bccsp.(type) {
	case *gm.CSP:
		return id.signer.Sign(rand.Reader, msg, &sm2.GMOpts{UID: nil})
	default:
