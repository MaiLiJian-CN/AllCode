...
func (msp *bccspmsp) getCertifiersIdentifier(certRaw []byte) ([]byte, error) {
	...
	case *gm.CSP:
		...
		for _, v := range msp.rootCerts {
			if v.(*identity).gmcert.Equal(cert) {
				found = true
				root = true
				break
			}
		...
		if root {
			...
			chain, err = msp.getGMValidationChain(cert, true)
			if err != nil {
		...
		certifiersIdentifier, err = msp.getGMCertificationChainIdentifierFromChain(chain)
		if err != nil {
...
func (msp *bccspmsp) setupGMCAs(conf *m.FabricMSPConfig) error {
	// make and fill the set of CA certs - we expect them to be there
	if len(conf.RootCerts) == 0 {
		return errors.New("expected at least one CA certificate")
	}
	...
	// CA certificates. After their sanitization is done, the opts
	// will be recreated using the sanitized certs.
	msp.gmopts = &sm2.VerifyOptions{Roots: sm2.NewCertPool(), Intermediates: sm2.NewCertPool()}
	for _, v := range conf.RootCerts {
		cert, err := msp.getGMCertFromPem(v)
		if err != nil {
			return err
		}
		msp.gmopts.Roots.AddCert(cert)
	}
	for _, v := range conf.IntermediateCerts {
		cert, err := msp.getGMCertFromPem(v)
		if err != nil {
		...
		msp.gmopts.Intermediates.AddCert(cert)
	}
	...
	msp.gmopts = &sm2.VerifyOptions{Roots: sm2.NewCertPool(), Intermediates: sm2.NewCertPool()}
	for _, id := range msp.rootCerts {
		msp.gmopts.Roots.AddCert(id.(*identity).gmcert)
	}
	for _, id := range msp.intermediateCerts {
		msp.gmopts.Intermediates.AddCert(id.(*identity).gmcert)
	}
...
func (msp *bccspmsp) finalizeSetupCAs() error {
	...
	case *gm.CSP:
		...
		for _, id := range append(append([]Identity{}, msp.rootCerts...), msp.intermediateCerts...) {
			if !id.(*identity).gmcert.IsCA {
				return errors.Errorf("CA Certificate did not have the CA attribute, (SN: %x)", id.(*identity).cert.SerialNumber)
			}
			if _, err := getGMSubjectKeyIdentifierFromCert(id.(*identity).gmcert); err != nil {
				return errors.WithMessage(err, fmt.Sprintf("CA Certificate problem with Subject Key Identifier extension, (SN: %x)", id.(*identity).gmcert.SerialNumber))
			}

			if err := msp.validateGMCAIdentity(id.(*identity)); err != nil {
				return errors.WithMessage(err, fmt.Sprintf("CA Certificate is not valid, (SN: %s)", id.(*identity).gmcert.SerialNumber))
			}
		}
		...
		for _, id := range append([]Identity{}, msp.intermediateCerts...) {
			chain, err := msp.getGMUniqueValidationChain(id.(*identity).gmcert, msp.getGMValidityOptsForCert(id.(*identity).gmcert))
			if err != nil {
				return errors.WithMessage(err, fmt.Sprintf("failed getting validation chain, (SN: %s)", id.(*identity).gmcert.SerialNumber))
			}
...
func (msp *bccspmsp) preSetupV1(conf *m.FabricMSPConfig) error {
	...
	switch msp.bccsp.(type) {
	case *gm.CSP:
		if err := msp.setupGMCAs(conf); err != nil {
			return err
		}
	default:
