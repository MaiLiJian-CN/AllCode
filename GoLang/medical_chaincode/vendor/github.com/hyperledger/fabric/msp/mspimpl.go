...
type bccspmsp struct {
	...
	// gm verification options for MSP members
	gmopts *sm2.VerifyOptions
...
func (msp *bccspmsp) getGMCertFromPem(idBytes []byte) (*sm2.Certificate, error) {
	if idBytes == nil {
...
func (msp *bccspmsp) getIdentityFromConf(idBytes []byte) (Identity, bccsp.Key, error) {
	// get a cert
	//switch sw and gm
	switch msp.bccsp.(type) {
	case *gm.CSP:
		cert, err := msp.getGMCertFromPem(idBytes)
		if err != nil {
			return nil, nil, err
		}
		...
		mspId, err := newGMIdentity(cert, certPubK, msp)
		if err != nil {
			return nil, nil, err
		}
...
func (msp *bccspmsp) getSigningIdentityFromConf(sidInfo *m.SigningIdentityInfo) (SigningIdentity, error) {
	...
	switch msp.bccsp.(type) {
	case *gm.CSP:
		return newGMSigningIdentity(idPub.(*identity).gmcert, idPub.(*identity).pk, peerSigner, msp)
	default:
...
func (msp *bccspmsp) DeserializeIdentity(serializedID []byte) (Identity, error) {
	...
	switch msp.bccsp.(type) {
	case *gm.CSP:
		return msp.deserializeGMIdentityInternal(sId.IdBytes)
	default:
...
// deserializeIdentityInternal returns an identity given its byte-level representation
func (msp *bccspmsp) deserializeGMIdentityInternal(serializedIdentity []byte) (Identity, error) {
	// This MSP will always deserialize certs this way
	bl, _ := pem.Decode(serializedIdentity)
	if bl == nil {
		return nil, errors.New("could not decode the PEM structure")
	...
	return newGMIdentity(cert, pub, msp)
}
...
func (msp *bccspmsp) satisfiesPrincipalInternalPreV13(id Identity, principal *m.MSPPrincipal) error {
	...
	case m.MSPPrincipal_ROLE:
		...
		case m.MSPRole_ADMIN:
			...
			if msp.isInAdmins(id.(*identity), isgm) {
				return nil
			}
	...
	case m.MSPPrincipal_IDENTITY:
		...
		if !isgm {
			...
			if bytes.Equal(id.(*identity).gmcert.Raw, principalId.(*identity).gmcert.Raw) {
				return principalId.Validate()
			}
		}
...
func (msp *bccspmsp) isInAdmins(id *identity, isgm bool) bool {
	for _, admincert := range msp.admins {
		if !isgm {
			...
			if bytes.Equal(id.gmcert.Raw, admincert.(*identity).gmcert.Raw) {
				return true
			}
		}
	}
...
// getCertificationChain returns the certification chain of the passed identity within this msp
func (msp *bccspmsp) getGMCertificationChain(id Identity) ([]*sm2.Certificate, error) {
	mspLogger.Debugf("MSP %s getting gm certification chain", msp.name)
	...
	// root of trust this MSP has
	case *identity:
		return msp.getGMCertificationChainForBCCSPIdentity(id)
	default:
		return nil, errors.New("identity type not recognized")
	}
}
...
// getCertificationChainForBCCSPIdentity returns the certification chain of the passed bccsp identity within this msp
func (msp *bccspmsp) getGMCertificationChainForBCCSPIdentity(id *identity) ([]*sm2.Certificate, error) {
	if id == nil {
		return nil, errors.New("Invalid bccsp identity. Must be different from nil.")
	}
	...
	return msp.getGMValidationChain(id.gmcert, false)
}
...
func (msp *bccspmsp) getGMUniqueValidationChain(cert *sm2.Certificate, opts sm2.VerifyOptions) ([]*sm2.Certificate, error) {
	// ask golang to validate the cert for us based on the options that we've built at setup time
	if msp.gmopts == nil {
		return nil, errors.New("the supplied identity has no verify options")
	}
...
func (msp *bccspmsp) getGMValidationChain(cert *sm2.Certificate, isIntermediateChain bool) ([]*sm2.Certificate, error) {
	validationChain, err := msp.getGMUniqueValidationChain(cert, msp.getGMValidityOptsForCert(cert))
	if err != nil {
		return nil, errors.WithMessage(err, "failed getting validation chain")
	}
...
func (msp *bccspmsp) getCertificationChainIdentifier(id Identity) ([]byte, error) {
	switch msp.bccsp.(type) {
	case *gm.CSP:
		chain, err := msp.getGMCertificationChain(id)
		if err != nil {
		...
		return msp.getGMCertificationChainIdentifierFromChain(chain[1:])
	default:
...
func (msp *bccspmsp) getGMCertificationChainIdentifierFromChain(chain []*sm2.Certificate) ([]byte, error) {
	// Hash the chain
	// Use the hash of the identity's certificate as id in the IdentityIdentifier
	hashOpt, err := bccsp.GetHashOpt(msp.cryptoConfig.IdentityIdentifierHashFunction)
	if err != nil {
...
// do have signatures in Low-S. If this is not the case, the certificate
// is regenerated to have a Low-S signature.
func (msp *bccspmsp) sanitizeGMCert(cert *sm2.Certificate) (*sm2.Certificate, error) {
	if isGMECDSASignedCert(cert) {
		// Lookup for a parent certificate to perform the sanitization
		var parentCert *sm2.Certificate
		chain, err := msp.getGMUniqueValidationChain(cert, msp.getGMValidityOptsForCert(cert))
		if err != nil {
			return nil, err
		}
		...
		// Sanitize
		cert, err = sanitizeGMECDSASignedCert(cert, parentCert)
		if err != nil {
			return nil, err
		}
	}
