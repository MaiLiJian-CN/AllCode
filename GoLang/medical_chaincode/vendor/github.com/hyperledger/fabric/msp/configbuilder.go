...
func GetLocalMspConfig(dir string, bccspConfig *factory.FactoryOpts, ID string) (*msp.MSPConfig, error) {
	...
	*/
	...
	return getMspConfig(dir, ID, sigid, bccspConfig.SwOpts.HashFamily)
}

// GetVerifyingMspConfig returns an MSP config given directory, ID and type
func GetVerifyingMspConfig(dir, ID, mspType string, hash string) (*msp.MSPConfig, error) {
	switch mspType {
	case ProviderTypeToString(FABRIC):
		return getMspConfig(dir, ID, nil, hash)
	case ProviderTypeToString(IDEMIX):
		return GetIdemixMspConfig(dir, ID)
	default:
		return nil, errors.Errorf("unknown MSP type '%s'", mspType)
...
func getMspConfig(dir string, ID string, sigid *msp.SigningIdentityInfo, HashFamily string) (*msp.MSPConfig, error) {
	...
	switch HashFamily {
	case bccsp.SHA2:
		cryptoConfig.SignatureHashFamily = bccsp.SHA2
		cryptoConfig.IdentityIdentifierHashFunction = bccsp.SHA256
	case bccsp.GMSM3:
		cryptoConfig.SignatureHashFamily = bccsp.GMSM3
		cryptoConfig.IdentityIdentifierHashFunction = bccsp.GMSM3
	default:
		cryptoConfig.SignatureHashFamily = bccsp.SHA2
		cryptoConfig.IdentityIdentifierHashFunction = bccsp.SHA256
	}
