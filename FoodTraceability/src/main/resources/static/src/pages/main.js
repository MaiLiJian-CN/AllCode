...
const Farm = {
  ...
  computed: {
    ...
    renderList() {
      ...
      // return this.foodList.slice(0,this.foodList.length);
      // return this.foodList.slice(1,2)
    }
      ...
      axios({
        ...
        url: `/producing`
          ...
          .then(ret => {
            ...
            /* 【11】请在此注释后作答，勿删除此注释*/
            this.foodList.forEach((value, index) => {

              value.traceNumber = ret.data[index].traceNumber
              value.foodName = ret.data[index].name
              value.traceName = ret.data[index].from
              value.date = dateTime(ret.data[index].timestamp)
              value.total=ret.data.length


            })


            this.a=getNum(this.foodList);



            this.foodList.reverse();

            this.$emit('Food', this.foodList.length);
    ...
    formSubmit(val) {
      ...
      .then(ret => {
        console.log(ret.data);
          ...
          // 已有溯源码校验
          if (ret.data.message === 'traceNumber already exist') {
            this.$message({
